...
router.post("/report-sheet-error", upload.single("helpingFile"), async (req, res) => {
    ...
    try {
        ...
        subject: "📊 Rapport d'erreurs de données",
        html: `
...
router.post("/report-soft-error", upload.single("helpingFile"), async (req, res) => {
    const { problem, userEmail, userName, userId } = req.body;
    const file = req.file;
